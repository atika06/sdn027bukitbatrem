...
if ($_FILES["img1"]["name"] != "") {
    ...
    $result = mysqli_query($koneksi, "SELECT img1 FROM galeri WHERE id = '$id'");
    ...
    $target_file = "img/galeri/" . $namaimg1Baru;
    move_uploaded_file($_FILES["img1"]["tmp_name"], $target_file);

    $response['body']['data']['img1'] = 'img/galeri/' . $namaimg1Baru;
    mysqli_query($koneksi, "UPDATE galeri SET img1 = '$namaimg1Baru' WHERE id = '$id'");
}
// cek apakah user pilih img2 baru atau tidak
if ($_FILES["img2"]["name"] != "") {
    // ambil nama img2 lama
    $result = mysqli_query($koneksi, "SELECT img2 FROM galeri WHERE id = '$id'");
    $data = mysqli_fetch_assoc($result);
    $img2 = $data['img2'];
    ...
    $target_file = "img/galeri/" . $namaimg2Baru;
    move_uploaded_file($_FILES["img2"]["tmp_name"], $target_file);

    $response['body']['data']['img2'] = 'img/galeri/' . $namaimg2Baru;
    mysqli_query($koneksi, "UPDATE galeri SET img2 = '$namaimg2Baru' WHERE id = '$id'");
}
...
mysqli_query($koneksi, "UPDATE galeri SET id = '$id', nama = '$nama', desk = '$desk' WHERE id = '$id'");
